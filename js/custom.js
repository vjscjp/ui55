...
function callListAppId()
{
    $.ajax({
        ...
        error: function() {
            callErrorDialog("Please login and Retry, You credentials expire","",true);
            sessionOut();
            siteView('L');
...
function callLogoutAPI()
{
    $.ajax({
        ...
        error: function() {
            callErrorDialog("User Already Logged Out.","Logout Marathon API",true);
            sessionOut();
            siteView('L');
            setTimeout(function(){$('#loader').hide();},500);
        },
        success: function(data, status, xhr) {
            callErrorDialog("Logging out....","Logout Marathon API",true);
            sessionOut();
            siteView('L');
...
function callLoginAPI(id,pass,url)
{
   var dataObj  = '{"id":"'+id+'","pass":"'+pass+'","api":"'+url+'"}';
    ...
    data: dataObj,
    error: function() {
      callErrorDialog("Something Went Wrong");
    },
    success: function(data, status, xhr) {
...
function callHostPortApi() {
    ...
    $.ajax({
        ...
        error: function() {
            callErrorDialog("Something went wrong, Please try again.");
            setTimeout(function(){$('#loader').hide();},500);
        },
        success: function(data, status, xhr) {
           renderApiData(data);
